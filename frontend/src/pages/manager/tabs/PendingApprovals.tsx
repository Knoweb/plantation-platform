import { Box, Typography, Card, CardContent, Table, TableHead, TableRow, TableCell, TableBody, Button, CircularProgress, Dialog, DialogTitle, DialogContent, TextField, DialogActions, Snackbar, Alert, IconButton, Chip, Grid, Avatar, ToggleButton, ToggleButtonGroup } from '@mui/material';
...
export default function PendingApprovals() {
    ...
    const fetchPending = async () => {
        ...
        try {
            const workRes = await axios.get(`http://localhost:8080/api/tenants/daily-work?tenantId=${tenantId}&status=${viewStatus}`);
            workItems = workRes.data.map((item: any) => ({
                id: item.workId,
                displayId: item.workId.substring(0, 8),
                type: item.workType,
                details: item.details && item.details.startsWith('[') ? 'Detailed Muster Plan' : `${item.divisionId} - ${item.details || ''}`,
        ...
        try {
            ...
            console.warn("Inventory service unavailable - skipping inventory items", e);
        }
    ...
    const handleDelete = async (item: any) => {
        ...
        try {
            if (item.source === 'WORK') {
                ...
                // Inventory delete/cancel if supported
            }
            fetchPending();
            ...
            <Card>
                ...
                <Table>
                    ...
                    <TableBody>
                        {pendingItems.map(item => (
                            <TableRow key={item.id}>
                                ...
                                <TableCell align="right">
                                    {/* Simplified Action: View Only for Muster (Approve/Reject inside) */}
                                    {item.source === 'WORK' ? (
                                        <Button
                                            size="small"
                                            variant="contained"
                                        ...
                                        <>
                                            <Button size="small" variant="outlined" color="error" sx={{ mr: 1 }} onClick={() => alert('Reject')}>Reject</Button>
                        ...
                        {pendingItems.length === 0 && <TableRow><TableCell colSpan={6} align="center">No pending approvals found.</TableCell></TableRow>}
                    </TableBody>
                </Table>
            </CardContent>
        </Card>
    ...
    {/* Unified Muster Review Dialog (Chit + Division View) */ }
    <Dialog open={Boolean(musterReviewItem)} onClose={() => setMusterReviewItem(null)} maxWidth="lg" fullWidth>
                    ...
                    <Box sx={{ maxHeight: '400px', overflowY: 'auto' }}>
                        {(() => {
                            try {
                                const details = musterReviewItem?.detailsRaw ? JSON.parse(musterReviewItem.detailsRaw) : [];
                                if (Array.isArray(details)) {
                                    // Group by Task for nicer display
                                                        ...
                                                        {assign.assigned && assign.assigned.length > 0 ? (
                                                            assign.assigned.map((w: any, wIdx: number) => (
                                                                <Chip
                                                                    ...
                                                                    label={w.name}
                                                                    size="small"
                                                                    avatar={<Avatar>{w.name.charAt(0)}</Avatar>}
                                                                />
                                                            ))
                                                        ) : (
        ...
        <DialogActions sx={{ bgcolor: '#e8f5e9', borderTop: '1px solid #c8e6c9', p: 2, justifyContent: 'space-between' }}>
            <Button variant="outlined" color="error" onClick={() => { alert("Reject Logic"); setMusterReviewItem(null); }}>
                Reject Muster
            </Button>
